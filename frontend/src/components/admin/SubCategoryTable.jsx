  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold text-gray-800 font-serif">Subcategories</h2>
        <Link
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Parent Category</th>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {subCategories.map((sub) => (
              <tr key={sub._id} className="hover:bg-gray-50 transition">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 line-clamp-1 font-mono text-[10px]">{sub.parent?.name || "N/A"}</td>
                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                  <div className="flex justify-end gap-3 text-gray-600">
                    <Link to={`/admin/subcategories/edit/${sub?._id}`} className="hover:text-blue-600">
                      <Edit size={18} />
                    </Link>
                    <button onClick={() => handleDelete(sub._id)} className="hover:text-red-600">
                      <Trash2 size={18} />
                    </button>
                  </div>
                </td>
            ...
            {subCategories.length === 0 && (
              <tr>
                <td colSpan="3" className="px-6 py-4 text-center text-sm text-gray-500">No subcategories found.</td>
              </tr>
            )}
          </tbody>
        </table>
